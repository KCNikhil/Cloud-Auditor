...
function App() {
  ...
  useEffect(() => {
    ...
    axios.get('http://127.0.0.1:8000/findings')
      .then(response => {
        ...
        // Calculate stats
        const critical = response.data.filter(f => f.Severity === 'Critical').length;
  ...
  return (
    <div style={{ padding: '40px', fontFamily: 'Arial, sans-serif', maxWidth: '1000px', margin: '0 auto' }}>
      ...
      {/* 2. The Chart */}
      <div style={{ height: '350px', marginBottom: '40px', border: '1px solid #ddd', padding: '20px', borderRadius: '8px' }}>
        ...
        <tbody>
          {findings.map((f) => (
            <tr key={f.FindingID} style={{ borderBottom: '1px solid #eee' }}>
              ...
              <td style={{ padding: '12px', color: '#666', fontSize: '0.9em' }}>{f.ResourceName}</td>
            </tr>
          ))}
        </tbody>
      </table>
